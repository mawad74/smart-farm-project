            ...
            <div class="col-md-6">
                <div class="card shadow-sm">
                    <div class="card-body">
                        ...
                        @endif
                        <form method="POST" action="{{ route('farmer_manager.alerts.update', $alert->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                                <label for="message" class="form-label">Message</label>
                                <input type="text" class="form-control @error('message') is-invalid @enderror" id="message" name="message" value="{{ old('message', $alert->message) }}" required>
                                @error('message')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error('date') is-invalid @enderror" id="date" name="date" value="{{ old('date', $alert->date) }}" required>
                                @error('date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
