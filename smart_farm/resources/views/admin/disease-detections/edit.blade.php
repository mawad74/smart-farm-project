            ...
            <div class="col-md-6">
                <div class="card shadow-sm">
                    <div class="card-body">
                        ...
                        @endif
                        <form method="POST" action="{{ route('admin.disease-detections.update', $detection->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                <label for="confidence" class="form-label">Confidence (0 to 1)</label>
                                <input type="number" step="0.01" min="0" max="1" class="form-control @error('confidence') is-invalid @enderror" id="confidence" name="confidence" value="{{ old('confidence', $detection->confidence) }}" required>
                                @error('confidence')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <button type="submit" class="btn btn-success">Update Detection</button>
                            <a href="{{ route('admin.disease-detections.index') }}" class="btn btn-secondary">Cancel</a>
                        </form>
                    </div>
                </div>
            </div>
